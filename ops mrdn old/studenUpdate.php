   ...
   <td ><div id="vertmenu">
     <ul>
       ...
       <li><a href="Logout.php">Logout</a></li>
        ...
        <TD COLSPAN=2 valign="top"><table width="100%" cellspacing="1" cellpadding="2">
          <tr>
            <td colspan="2"><h2 align="center" class="txt2">Update Student Result Page </h2></td>
          </tr>
          <tr>
...
$id = mysql_real_escape_string($_GET['id']);
		...
		$qry = "SELECT * FROM position WHERE id='$id'";
		
		$res = mysql_query($qry) or die("Can not Execute Your Query");
...
//////////////////////////////////////////////////////////
        ...
        if(empty($class)){
							$errors[] = 'The class  must not be empty.';
												}
												
			  if(empty($obtmarks)){
							$errors[] = 'The obtain marks  must not be empty.';
												}
			...
			if(empty($status)){
							$errors[] = 'The status must not be empty.';
												}
												
	if(empty($position)){
							$errors[] = 'The position must not be empty.';
												}
	if(empty($year)){
...
/////////////////////////////////////////////////////////////////////////////
			...
			if(!empty($errors)) 
	{
		...
		if(move_uploaded_file($_FILES["fileupload"]["tmp_name"], $uploaddir.$_FILES["fileupload"]["name"]))
						...
						@chmod($path,0777);
$qry = "UPDATE position SET picture ='$path', name='$name',class='$class',mobtain='$obtmarks', tmarks='$tmarks', status='&status', position='$position', year='$year'  WHERE id ='$id'"; 
					
					$res=mysql_query($qry) or die("Can Not Add Ur Record".mysql_error());
									
										if($res){
										$flag="All Records Was Added & Picture Was Successfully Replaced !";
										echo $flag;
													}					
...
?>
          ...
          <tr>
            ...
            <td width="72%"><input name="name" type="text" id="name" size="30" value="<?php echo $name; ?>"/></td>
          </tr>
          <tr>
            <td valign="top" class="txt2"><div align="center">Student  Class </div></td>
            <td><input name="class" type="text" id="class" size="30" value="<?php echo $class; ?>"/></td>
          </tr>
          <tr>
            <td class="txt2"><div align="center">Obtained Marks </div></td>
            <td><input name="obtmarks" type="text" id="obtmarks" size="30"  value="<?php echo $obtmarks; ?>"/></td>
          </tr>
          <tr>
            <td class="txt2"><div align="center">Total Marks </div></td>
            <td><input name="tmarks" type="text" id="tmarks" size="30" value="<?php echo $tmarks; ?>"/>
            </td>
          </tr>
          <tr>
            <td class="txt2"><div align="center">Student Picture </div></td>
            <td><input name="fileupload" type="file" id="fileupload" size="30" /></td>
          </tr>
          <tr>
          ...
          <tr>
            <td>&nbsp;</td>
            <td><input name="Submit" type="submit" class="but" value="Add Student Results" />
              &nbsp;&nbsp;&nbsp;
